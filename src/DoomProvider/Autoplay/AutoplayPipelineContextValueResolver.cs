...
internal static class AutoplayPipelineContextValueResolver
{
    ...
    /// </param>
    /// <param name="token">
    /// [EN] Supplies the <c>token</c> value for the Doom integration operation.
    /// [JA] Doom integration operation に渡す <c>token</c> value です。
    /// </param>
    /// <returns>
    ...
    public static AutoplayDslValue Resolve(AutoplayPipelineContext context, string token)
    {
        ...
        return normalized.ToLowerInvariant() switch
        {
            ...
            "enemystructuraldecoy" => AutoplayDslValue.Boolean(context.EnemyStructuralDecoy),
            "enemyconfidencepeak" => AutoplayDslValue.Number(context.EnemyConfidencePeak),
            "trustedenemythreat" => AutoplayDslValue.Number(context.TrustedEnemyThreat),
            "trustedcombatevidence" => AutoplayDslValue.Boolean(context.TrustedCombatEvidence),
            "postdoorenemymemoryevidence" => AutoplayDslValue.Boolean(context.PostDoorEnemyMemoryEvidence),
