...
public sealed class DoomWebGuiTests
{
    ...
    [Fact]
    public void DoomWeb_StagesCanonicalCanonicalBrowserBridgeAssets()
    {
        ...
        Assert.Contains("runtime\\browser\\webgpu-envelope-bridge.js", project, StringComparison.Ordinal);
        Assert.Contains("shaders\\hud\\hud-composite.wgsl", project, StringComparison.Ordinal);
        Assert.Contains("wwwroot\\js\\aikernel\\webgpu-envelope-bridge.js", project, StringComparison.Ordinal);
        Assert.Contains("wwwroot\\demo\\doom\\js\\aikernel\\webgpu-envelope-bridge.js", project, StringComparison.Ordinal);
        Assert.Contains("window.AIKernelDoomGpuAssetBase", publicLoader, StringComparison.Ordinal);
        Assert.Contains("window.AIKernelWebGpuReady = import(moduleUrl)", publicLoader, StringComparison.Ordinal);
        Assert.Contains("createWebGpuEnvelopeBridge: module.createWebGpuEnvelopeBridge", publicLoader, StringComparison.Ordinal);
        Assert.Contains("self.AIKernelDoomGpuAssetBase = `${workerScriptBase}aikernel/`", worker, StringComparison.Ordinal);
        Assert.Contains("self.AIKernelWebGpuReady = import(scriptUrl(\"/js/aikernel/webgpu-envelope-bridge.js\"))", worker, StringComparison.Ordinal);
        Assert.Contains("createWebGpuBrowserExecutor: module.createWebGpuBrowserExecutor", worker, StringComparison.Ordinal);
    }
    ...
    public void DoomWebGpuProvider_SeparatesRawFramebufferAndHudCompositeTargets()
    {
        ...
        Assert.Contains("const HUD_OFFSCREEN_COMPOSITE_ENABLED = true", script, StringComparison.Ordinal);
        Assert.Contains("const HUD_OFFSCREEN_COMPOSITE_MAX_FPS = 10", script, StringComparison.Ordinal);
        Assert.Contains("hudOffscreenCompositeRuntimeDisabled", script, StringComparison.Ordinal);
        Assert.Contains("hudOffscreenCompositeFailureCount", script, StringComparison.Ordinal);
        Assert.Contains("HUD offscreen composite disabled for this session", script, StringComparison.Ordinal);
        Assert.Contains("const GPU_HUD_COMPOSITE_PILOT_TARGET = `${HUD_COMPOSITE_TARGET}:canonical-pilot`", script, StringComparison.Ordinal);
        ...
        Assert.Contains("doom.hud.composite.canonical-pilot", script, StringComparison.Ordinal);
        ...
        Assert.Contains("canonicalHudCompositePilotTexture", script, StringComparison.Ordinal);
        Assert.Contains("hudCompositePresentBindGroups", script, StringComparison.Ordinal);
        Assert.Contains("doom.hud.composite.overlay.pass", script, StringComparison.Ordinal);
        Assert.Contains("doom.present.hud.single-pass", script, StringComparison.Ordinal);
        Assert.Contains("doom.present.raw.base.pass", script, StringComparison.Ordinal);
        ...
        Assert.Contains("displayTarget: FRAME_TARGET", script, StringComparison.Ordinal);
        Assert.Contains("const SWAPCHAIN_HUD_SINGLE_PASS_WIRE_NAME = `${RAW_FRAMEBUFFER_WIRE_NAME}+gpu-hud-single-pass`", script, StringComparison.Ordinal);
        Assert.Contains("displaySource: hudSwapchainActive ? SWAPCHAIN_HUD_SINGLE_PASS_WIRE_NAME : RAW_FRAMEBUFFER_WIRE_NAME", script, StringComparison.Ordinal);
        Assert.Contains("hudCompositeDisplayMode: hudSwapchainActive ? \"single-pass\" : \"raw\"", script, StringComparison.Ordinal);
        ...
        Assert.Contains("cssOverlayMode: hudSwapchainActive ? \"reduced\" : \"full\"", script, StringComparison.Ordinal);
        ...
        Assert.Contains("const GPU_SHADER_PATHS = Object.freeze", script, StringComparison.Ordinal);
        Assert.Contains("\"gpu.hud.composite\": \"shaders/hud-composite.wgsl\"", script, StringComparison.Ordinal);
        Assert.Contains("\"gpu.aisthesis.raw-frame\": \"shaders/aisthesis.wgsl\"", script, StringComparison.Ordinal);
        Assert.Contains("\"gpu.spatial-reasoning\": \"shaders/spatial-reasoning.wgsl\"", script, StringComparison.Ordinal);
        Assert.Contains("async initializeCanonicalBridge()", script, StringComparison.Ordinal);
        Assert.Contains("createWebGpuEnvelopeBridge", script, StringComparison.Ordinal);
        Assert.Contains("createWebGpuBrowserExecutor", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalTextureRegistry()", script, StringComparison.Ordinal);
        Assert.Contains("resolveCanonicalTexture(targetId)", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalHudCompositeEnvelope(renderer)", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalAisthesisEnvelope(renderer)", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalSpatialEnvelope(renderer)", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalMatrixBuffers()", script, StringComparison.Ordinal);
        Assert.Contains("scheduleCanonicalHudCompositePilot(renderer)", script, StringComparison.Ordinal);
        Assert.Contains("scheduleCanonicalAisthesisPilot(renderer)", script, StringComparison.Ordinal);
        Assert.Contains("scheduleCanonicalSpatialPilot(renderer)", script, StringComparison.Ordinal);
        Assert.Contains("this.canonicalBridge.dispatchAisthesisEnvelope(envelope)", script, StringComparison.Ordinal);
        Assert.Contains("this.canonicalBridge.dispatchSpatialReasoningEnvelope(envelope)", script, StringComparison.Ordinal);
        Assert.Contains("this.canonicalBridge.dispatchHudCompositeEnvelope(envelope)", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalAisthesisSummary(", script, StringComparison.Ordinal);
        Assert.Contains("function canonicalDiagnosticsMetadata", script, StringComparison.Ordinal);
        Assert.Contains("featureMaskStorageTexture", script, StringComparison.Ordinal);
        Assert.Contains("feature_mask_storage_texture", script, StringComparison.Ordinal);
        Assert.Contains("createCanonicalSpatialSummary(", script, StringComparison.Ordinal);
        ...
        Assert.Contains("createCanonicalPilotParityHistory(", script, StringComparison.Ordinal);
        Assert.Contains("updateCanonicalPilotParityHistory(", script, StringComparison.Ordinal);
        Assert.Contains("GPU_AISTHESIS_PARITY_THRESHOLDS", script, StringComparison.Ordinal);
        Assert.Contains("GPU_SPATIAL_PARITY_THRESHOLDS", script, StringComparison.Ordinal);
        Assert.Contains("GPU_AISTHESIS_PROMOTION_STREAK_REQUIRED", script, StringComparison.Ordinal);
        Assert.Contains("GPU_SPATIAL_DIAGNOSTIC_STREAK_REQUIRED", script, StringComparison.Ordinal);
        Assert.Contains("canonicalBridgeReady: Boolean(canonicalBridge.bridgeAvailable && canonicalBridge.executorAvailable)", script, StringComparison.Ordinal);
        Assert.Contains("canonicalShaderSourcesReady: Boolean(canonicalBridge.shaderSourcesReady)", script, StringComparison.Ordinal);
        Assert.Contains("canonicalTextureRegistryReady: Boolean(canonicalBridge.textureRegistryReady)", script, StringComparison.Ordinal);
        Assert.Contains("canonicalAisthesisPilotSucceeded", script, StringComparison.Ordinal);
        Assert.Contains("canonicalAisthesisSummary", script, StringComparison.Ordinal);
        Assert.Contains("canonicalAisthesisParityHistory", script, StringComparison.Ordinal);
        Assert.Contains("canonicalSpatialPilotSucceeded", script, StringComparison.Ordinal);
        Assert.Contains("canonicalSpatialSummary", script, StringComparison.Ordinal);
        Assert.Contains("canonicalSpatialParityHistory", script, StringComparison.Ordinal);
        Assert.Contains("canonicalHudCompositePilotTextureReady", script, StringComparison.Ordinal);
        Assert.Contains("canonicalHudCompositePilotSucceeded", script, StringComparison.Ordinal);
        ...
        Assert.Contains("hudOffscreenCompositeMaxFps: HUD_OFFSCREEN_COMPOSITE_MAX_FPS", script, StringComparison.Ordinal);
        Assert.Contains("maxFps: HUD_OFFSCREEN_COMPOSITE_MAX_FPS", script, StringComparison.Ordinal);
    ...
    public void DoomPrompt_UsesCanonicalAisthesisNoesisKrisisKinesisPanelMapping()
    {
        ...
        Assert.Contains("function canonicalPilotText", gpuPathStatus, StringComparison.Ordinal);
        Assert.Contains("function featureMaskStorageTexture", gpuPathStatus, StringComparison.Ordinal);
        Assert.Contains("function passReadinessValue", gpuPathStatus, StringComparison.Ordinal);
        Assert.Contains("function passReadinessText", gpuPathStatus, StringComparison.Ordinal);
        ...
        Assert.Contains("pass_id", gpuPathStatus, StringComparison.Ordinal);
        Assert.Contains("pass_readiness", gpuPathStatus, StringComparison.Ordinal);
        Assert.Contains("feature_mask_storage_texture", gpuPathStatus, StringComparison.Ordinal);
        ...
        Assert.Contains("canonicalAisthesis", gpuPathStatus, StringComparison.Ordinal);
        Assert.Contains("canonicalSpatial", gpuPathStatus, StringComparison.Ordinal);
        ...
        Assert.Contains("pilot_state", objectiveStatusPanel, StringComparison.Ordinal);
        Assert.Contains("pass_readiness", objectiveStatusPanel, StringComparison.Ordinal);
        Assert.Contains("feature_mask_storage_texture", objectiveStatusPanel, StringComparison.Ordinal);
        ...
        Assert.Contains("pilotState", objectiveStatusPanelVmTest, StringComparison.Ordinal);
        Assert.Contains("passReadiness", objectiveStatusPanelVmTest, StringComparison.Ordinal);
        Assert.Contains("featureMaskStorageTexture", objectiveStatusPanelVmTest, StringComparison.Ordinal);
    ...
    public void DoomRuntime_SeparatesDoomRetryDispatchStateMachine()
    {
        ...
        Assert.Contains("scheduleAutoplayFrameRetryDispatch(state)", runtime, StringComparison.Ordinal);
        Assert.Contains("neutralizeAutoplayForRetrySignal(`retry-${scheduled.reason}`)", runtime, StringComparison.Ordinal);
        Assert.Contains("retry dispatch queued before prediction", runtime, StringComparison.Ordinal);
        Assert.True(
            runtime.IndexOf("this.scheduleAutoplayFrameRetryDispatch(state)", StringComparison.Ordinal) <
            runtime.IndexOf("this.updateGpuHudOverlayState(state);", StringComparison.Ordinal));
    ...
    public void DoomAutoplayCognition_ModularizesToposCtgAndKinesisCarriers()
    {
        ...
        Assert.Contains("deathTintScore", sensory, StringComparison.Ordinal);
        Assert.Contains("health-red-tint-death", sensory, StringComparison.Ordinal);
        ...
        Assert.Contains("scoreDeathTintPaletteIndex", visionPalette, StringComparison.Ordinal);
    ...
    public void DoomBonsai_DelegatesFirstDoorPriorityContextToKairosModule()
    {
        ...
        Assert.Contains("return requireVisionPalette(\"scoreDeathTintPaletteIndex\")(index, rgbaBytes);", bonsai, StringComparison.Ordinal);
        ...
        Assert.Contains("function scoreDeathTintPaletteIndex(index, rgbaBytes)", visionPalette, StringComparison.Ordinal);
