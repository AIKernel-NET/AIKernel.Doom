...
assert(gpuPath?.version === "20260621-gpupathstatus8", "GPU path status module should expose the current version");
assert(typeof gpuPath.resolveGpuPathStatus === "function", "GPU path status module should expose resolver");
assert(context.self.document.documentElement.dataset.doomGpuPathStatusVersion === "20260621-gpupathstatus8", "GPU path status module should publish its live diagnostics version");
...
assert(healthy.rows.some(row => row.label === "SENSOR" && row.value.includes("mask=gpu")), "SENSOR row should expose mask texture readiness");
